    ...
    <modal ref="form" header-height="tiny">
      ...
      <template v-slot:main>
        <div class="new-product-form vertical p-4" v-if="!$empty($v)">
          <form-input v-model="$v.product.name.$model">Titulo</form-input>
          <form-validation :error="$v.product.name.$error" class="mb-4">O nome do formato não pode estar vazio</form-validation>
          <form-textarea v-model="product.description" class="mb-4">Descrição</form-textarea>
          <div class="row mb-5">
            <div class="vertical middle center col-sm-6">
              <form-money v-model="$v.product.price.$model" class="full-width" placeholder="100,00">
                Preço para Contratar
              </form-money>
              <form-validation :error="$v.product.price.$error" class="mb-4">Entre com um preço válido</form-validation>
            </div>
            <div class="vertical middle center col-sm-6">
              <form-numeric v-model="$v.product.duration.$model" class="full-width" icon="clock" placeholder="4:00">
                Duração da Apresentação (horas)
              </form-numeric>
          ...
          <div class="vertical middle mb-5">
            <div class="mb-5">
              ...
              <div v-for="(item, itemIndex) in product.items" :key="itemIndex" class="items d-flex justify-content-between">
                <span>{{ item }}</span>
                <icon icon="times" class="clickable" @click="removeItem(itemIndex)"></icon>
              </div>
            </div>
            ...
            <div class="horizontal middle full-width mb-2">
              <form-input v-model="newMedia.url" class="full-width" placeholder="Cole o link de suas midias sociais aqui">
              </form-input>
            ...
            <fade-transition group class="mb-5">
              <div v-for="(media, mediaIndex) in product.medias" :key="mediaIndex">
                <media-thumbnail class="mb-2" simple removable :media="media" @remove="removeMedia(mediaIndex)">
                </media-thumbnail>
              </div>
            </fade-transition>
            <div class="horizontal d-flex justify-content-between mb-2">
              <h6>Adicionar documentos</h6>
              <icon icon="plus" class="ml-2 clickable" @click="uploadDocument"></icon>
            </div>
      ...
      <template v-slot:footer>
        <div class="horizontal center middle my-4">
          ...
          <div v-if="!$empty(product.id)" class="brand-hover clickable ml-4" @click="openConfirmModal">
            Remover formato
          </div>
        </div>
      </template>
    </modal>
    <confirm-dialog ref="confirm" yes-no message="Excluir mesmo assim?" @confirmed="remove">
    </confirm-dialog>    
  </div>
</template>
...
export default {
  ...
  methods: {
    ...mapActions('artist', ['saveProduct', 'removeProduct']),
    newProduct() {
    ...
    openConfirmModal() {
      const self = this;
      this.$toast.info('Remover o formato não irá cancelar propostas ou apresentações associadas a ele. Você deve manualmente rejeitá-las ou redefini-las com seus clientes.');
      setTimeout(() => self.$refs.confirm.show(), 6000);
