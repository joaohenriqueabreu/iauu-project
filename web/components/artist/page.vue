<template>
  <div>
    ...
    <div class="container">
      ...
      <div v-if="!$empty(feedbacks)" class="mb-5 mx-5">
        <h4 class="mb-4">O que falam sobre nosso show?</h4>
        <div v-for="(feedback, index) in feedbacks" :key="index" class="horizontal">
...
import Feedback         from '@/components/artist/profile/feedback';
...
export default {
  ...
  async mounted() {
    ...
    await this.loadArtistFeedbacks(this.artist.id);
  },
  computed: {
    ...mapState({ artist: (state) => state.artist.artist }),
    ...mapState({ feedbacks: (state) => state.feedback.feedbacks }),
    socialMedias() {
  ...
  methods: {
    ...
    ...mapActions('feedback', ['loadArtistFeedbacks']),
